...
export async function email_taken(db: DatabaseSync, email: string){
   const prepared = db.prepare(`select email from users where email=:email`);
...
export async function new_user(db: DatabaseSync, {name, email, password}: User){
   ...
   const prepared = db.prepare(`insert into users (name, email, password) values (:name, :email, :password)`);
